...
bool j1App::Awake()
{
	...
	pugi::xml_parse_result LoadStuff = config.load_file("xml_docs/Configuration.xml");
	if (LoadStuff) {
		LOG("configuration.xml loaded correctly.");
	}
	else {
		LOG("Configuration.xml did not load correctly.");
		LOG(LoadStuff.description());
		return false;
	}
	bool ret = true;
